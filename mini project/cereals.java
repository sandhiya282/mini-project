...
public class CerealsGrains extends JFrame {
    ...
    public CerealsGrains() {
        setTitle("Cereals and Grains");
        setSize(800, 500);
        setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        setLocationRelativeTo(null);

        fruitsvegPanel = new JPanel(new BorderLayout());
        fruitsvegPanel.setBackground(new Color(173, 216, 230)); // Background color
    ...
    private void loadDataFromDatabase() {
        try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD);
             ...
             ResultSet resultSet = statement.executeQuery("SELECT * FROM cerealsgrains")) {
    ...
    private void addRecord() {
        try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD)) {
            String sql = "INSERT INTO cerealsgrains (id, name, availability) VALUES (?, ?, ?)";
    ...
    private void editRecord() {
        try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD)) {
            String sql = "UPDATE cerealsgrains SET name = ?, availability = ? WHERE id = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
    ...
    private void removeRecord() {
        try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD)) {
            String sql = "DELETE FROM cerealsgrains WHERE id = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setString(1, idField.getText());
            preparedStatement.executeUpdate();
            loadDataFromDatabase();
