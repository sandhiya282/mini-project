...
public class MeatPoultry extends JFrame {
    ...
    public MeatPoultry() {
        setTitle("Meat and Poultry");
        setSize(800, 500);
        setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        setLocationRelativeTo(null);

        MpPanel = new JPanel(new BorderLayout());
        MpPanel.setBackground(new Color(173, 216, 230)); // Background color
        ...
        MpPanel.add(formPanel, BorderLayout.WEST);

        // Center table
        table = new JTable();
        loadDataFromDatabase(); // Load data from DB
        JScrollPane tableScrollPane = new JScrollPane(table);
        MpPanel.add(tableScrollPane, BorderLayout.CENTER);
        ...
        MpPanel.add(buttonPanel, BorderLayout.SOUTH);
    ...
    private void loadDataFromDatabase() {
        try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD);
             ...
             ResultSet resultSet = statement.executeQuery("SELECT * FROM meatpoultry")) {
    ...
    private void addRecord() {
        try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD)) {
            String sql = "INSERT INTO meatpoultry (id, name, availability) VALUES (?, ?, ?)";
    ...
    private void editRecord() {
        try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD)) {
            String sql = "UPDATE meatpoultry SET name = ?, availability = ? WHERE id = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
    ...
    private void removeRecord() {
        try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD)) {
            String sql = "DELETE FROM meatpoultry WHERE id = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setString(1, idField.getText());
            preparedStatement.executeUpdate();
            loadDataFromDatabase();
