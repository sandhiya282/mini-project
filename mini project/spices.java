...
public class Spices extends JFrame {
    ...
    public Spices() {
        ...
        spicesPanel = new JPanel(new BorderLayout());
        spicesPanel.setBackground(new Color(173, 216, 230)); // Background color
        ...
        spicesPanel.add(formPanel, BorderLayout.WEST);

        // Center table
        table = new JTable();
        loadDataFromDatabase(); // Load data from DB
        JScrollPane tableScrollPane = new JScrollPane(table);
        spicesPanel.add(tableScrollPane, BorderLayout.CENTER);
        ...
        spicesPanel.add(buttonPanel, BorderLayout.SOUTH);
    ...
    private void loadDataFromDatabase() {
        try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD);
             ...
             ResultSet resultSet = statement.executeQuery("SELECT * FROM spices")) {
    ...
    private void addRecord() {
        try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD)) {
            String sql = "INSERT INTO spices (id, name, availability) VALUES (?, ?, ?)";
    ...
    private void editRecord() {
        try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD)) {
            String sql = "UPDATE spices SET name = ?, availability = ? WHERE id = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
